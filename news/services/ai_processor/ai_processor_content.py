...
""".strip()
    ...
    def generate_full_content(self, content: str, language: str) -> str:
        """Генерує повний Business Impact контент (1200-1500 символів) певною мовою."""
        try:
            prompt = f"""
...
на основі наступної статті (1200-1500 символів, практичний фокус, адаптовано для {language} ринку):

{content}

ВИМОГИ ДО ЯКОСТІ:
- Якість публікації повинна виносити від 8 до 10 балів за шкалою журналістської майстерності
- В результаті аналізу публікації, підбери оптимальний стиль та розмір публікації
- Якщо це необхідно, можеш написати публікацію коротким текстом, але якщо є потреба розкрити більше важливих деталей без води - розмір публікації від 1200 до 1500 символів, але в публікації потрібно передати як можна більше змісту

СТИЛІСТИЧНІ ВИМОГИ:
- Додай emoji в залежності від контексту абзацу, на початку абзацу в якому висвітлюється важлива інформація
- Використай спокійний стиль написання, але використай нейробіологічний аналіз тексту, та використай трігерні словосполучення, які активують у читача частину головного мозку "cortex"
- Додавай пустий рядок між абзацами
- В публікації додавай цитати та пряму мову, якщо в тексті такі присутні
